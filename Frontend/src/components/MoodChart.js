...
const MoodChart = ({ moodData, width = 1000, height = 350, showTimeBelowDate = false }) => {
  if (!moodData || moodData.length === 0) {
    return <div className="text-center text-gray-500">No mood data available</div>;
  }

  const padding = 40;
  const extraBottom = showTimeBelowDate ? 90 : 55;
  ...
  return (
    <svg width={width} height={height} style={{ background: '#fff', borderRadius: 8, boxShadow: '0 1px 4px #0001' }}>
      ...
      <text
  x={padding - 30}
  y={(height - extraBottom) / 2 + 10}  // moved down by 10 pixels
  fontSize="14"
  textAnchor="middle"
  fill="#222"
...
>
  Mood (Negative → Positive)
      ...
      {/* Date and time labels, horizontal */}
      {points.map((p, i) => (
        <g key={i}>
          <text
            x={p.x}
